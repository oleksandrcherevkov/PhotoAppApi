...
namespace PhotoAppApi.Services.Posts.QueryObjects
{
    ...
    {
        public static IQueryable<PostListDto> SelectListDto(this IQueryable<Post> query, string currentUserLogin, string host)
        {
            return query.Select(p => new PostListDto()
            {
                Id = p.Id,
                CreatorLogin = p.CreatorLogin,
                CreatorAvatar = p.Creator.Avatar == null ? "" : $"https://{host}/api/photo/avatar/{p.Creator.Avatar.Id}/{p.Creator.Avatar.Name}",
                Title = p.Title,
                Description = p.Description ?? String.Empty,
                Photo = $"https://{host}/api/photo/post/{p.Photo.Id}/{p.Photo.Name}",
